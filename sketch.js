...
function draw() {
  switch (escena) {
    case 0: //------OSCILOSCOPIO----------------------
      ...
      for (let x = 0; x < osciloscopio; x += 35) {//c/cuanto la linea        
       
        let grosor = random(70); //random grosor
        strokeWeight(grosor);
        line(x, width / 2, x + grosor, width / 2);
        if (contador > 70) { //el osciloscopio empieza a desaparecer p/que la transcisión sea + orgánica
          osciloscopio--;
        }
      }
  ...
  angulo += 0.07;
  // console.log("contador: "+ contador +" valor de angulo: " + angulo);
  contador++;
